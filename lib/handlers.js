...
handlers.checks = (data, callback) => {
    ...
    let method = (data.method || '').toLowerCase();
    if (acceptableMethods.indexOf(method) > -1) {
        handlers.checks[method](data, callback);
    } else {
        callback(405); // method is not allowed
    }
};
        ...
        _data.read('tokens', token, (err, tokenData) => {
            if (!err && tokenData) {
                ...
                _data.read('users', userPhone, (err, userData) => {
                    if (!err && userData) {
                        let userChecks = typeof userData.checks == 'object' && userData.check instanceof Array ? userData.checks : []
                        
                        // TODO add checks code logic
                    } else {
                        callback(403);
                    }
                });
